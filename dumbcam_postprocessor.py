...
class DumbCAMPostProcessor:
    def __init__(self, material_thickness: float, tool_diameter: float, units: str = "inch",
        ...
        Args:
            ...
            config: Optional TeamConfig instance for team-specific settings.
                   If not provided, uses DumbCAM defaults.
        """
        # Use provided config or create default (DumbCAM defaults)
        if config is None:
            config = TeamConfig()
        self.config = config
    ...
    def apply_material_preset(self, material: str, machine_id: Optional[str] = None):
        ...
        """
        # Get material preset from config (merges user config with DumbCAM defaults)
    ...
    def _generate_gcode_header(self, timestamp: str = None, is_multilayer: bool = False) -> List[str]:
        ...
        if is_multilayer:
            gcode.append("(DumbCAM CNC Post-Processor - MULTI-LAYER)")
        else:
            gcode.append("(DumbCAM CNC Post-Processor)")
...
def main():
    parser = argparse.ArgumentParser(description='DumbCAM Post-Processor')
